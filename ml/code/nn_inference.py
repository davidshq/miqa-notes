...
class TiledClassifier(monai.networks.nets.Classifier): # A NN module (torch.nn) that allows only one output for any input pattern
    '''Defines how data is passed through NN.'''
    def forward(self, inputs):
        ...
        # Divides the size of the input by the tile size,
        # We need to ensure we have enough steps to cover
        # the entire input with tiles
        z_steps = math.ceil(z_size / z_tile_size)
        y_steps = math.ceil(y_size / y_tile_size)
        x_steps = math.ceil(x_size / x_tile_size)

        # Iterate until one has covered the entire image
        for k in range(z_steps):
            # starting index of a tile in the z-axis
            k_start = round(k * (z_size - z_tile_size) / max(1, z_steps - 1))
            # index of tile on the y-axis
            for j in range(y_steps):
                j_start = round(j * (y_size - y_tile_size) / max(1, y_steps - 1))
                # index of tile on the x axis
                for i in range(x_steps):
                    ...
                    # Tile is larger than input size, we need to pad
                    if x_pad + y_pad + z_pad > 0:
                        # (input, (last_dim_left, last_dim_right, prior_dim_left...), mode)
...
def get_model(file_path=None):
    ...
    model = TiledClassifier(
        in_shape=(1, 64, 64, 64), # input_shape
        # regression_count is statically set to 1
        classes=regression_count + len(artifacts), # number of classes = number of outputs for any given input pattern
        channels=(4, 8, 16, 32, 64), # filters used to extract features from images
        strides=(2, 2, 2, 2, 2), # how many pixels to move over when applying a filter to an image (the higher this value, the less overlap there will be between tiles)
        dropout=0.1, # Dropout is a regularization technique that prevents overfitting by randomly setting some weights to zero during training (this forces other nodes to pick up slack)
    )

    # if the file exists
    if file_path is not None:
        # " A state_dict is simply a Python dictionary object that maps each layer to its parameter tensor."
        model.load_state_dict(torch.load(file_path, map_location=device)) # map_location forces to use CPU even if file was saved on GPU
...
#
# Note: Essentially an affine matrix describes how to
...
# Takes in an affine matrix and returns origin, spacing, and direction
# of the image.
...
# Convert image from TorchIO to ITK
...
# ITK library converts to numpy array
# numpy array converted to a tensor using torchio
...
# Rescales the intensity of an image
class ReorientAndRescale(torchio.transforms.RescaleIntensity):
    def apply_transform(self, subject: torchio.Subject) -> torchio.Subject:
        ...
        # Gets a numpy array of image using ITK from torchio image
...
# If num > max_value, return max_value, else return num as min
# If min > min_value, return min, else return min_value
...
def evaluate_model(model, data_loader, device, writer, epoch, run_name):
    ...
    with torch.no_grad():
        ...
        for val_data in data_loader:
            ...
            # Take all the rows but skip first column
            ...
            # Add the outputs to y_all
            # ... is literal ellipsis indicating slice operator should
            # take all dimensions of outputs
            y_all.extend(outputs[..., :].cpu().tolist())
            # Add the outputs to y_true
            ...
            # Limits values to a min of 0 and max of 10
            y = [clamp(y[t], 0, 10) for t in range(len(y))]
            y_pred.extend(y)
            # output_artifact is a copy of the original tensor
            # but without gradient=True
            output_artifact = outputs[..., 1:].detach().cpu()
            y_a = output_artifact.numpy()
            y_a = np.rint(y_a)
            # If y_a is < 0, set y_a = 0, if y_a > 1, set y_a = 1
            y_a = np.clip(y_a, 0, 1)
...
def label_results(result):
    # Divide the result by 10, if < 0.0, set to 0.0, if > 1.0 set to 1.0
    labeled_results = {'overall_quality': clamp(result[0] / 10.0, 0.0, 1.0)}
    # It takes the last three values of the result?
...
def evaluate1(model, image_path):
    ...
    evaluation_ds = monai.data.Dataset(
        data=[
            # A subject represents an individual
            torchio.Subject(
                {
                    'img': torchio.ScalarImage(image_path),
                    # creates a list of zeroes of the given length
